...
// Override fread_ to simulate partial read failure
// This strong symbol overrides the weak symbol in wrapper.c
size_t fread_(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
    ...
    (void)nmemb;
    (void)stream;
    // Return 0 to simulate read failure
    return 0;
}

// Test: fread returns fewer bytes than expected
START_TEST(test_migration_fread_partial_failure) {
    ...
    ck_assert_ptr_nonnull(strstr(msg, "Failed to read migration file"));
...
{
    Suite *s = suite_create("db_migration_fread");

    TCase *tc_core = tcase_create("Core");
    tcase_set_timeout(tc_core, IK_TEST_TIMEOUT);
    tcase_add_checked_fixture(tc_core, migration_test_setup, NULL);
    tcase_add_test(tc_core, test_migration_fread_partial_failure);
...
int main(void)
{
    ...
    srunner_set_xml(sr, "reports/check/unit/apps/ikigai/db/zzz_migration_fread_test.xml");
