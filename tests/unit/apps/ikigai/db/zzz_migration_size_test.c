...
// Mock ftell that returns LONG_MAX to trigger size check
long ftell_(FILE *stream)
{
    (void)stream;
    // Return a value that will cause file_size > UINT_MAX - 1
    return LONG_MAX;
}

START_TEST(test_migration_file_too_large) {
...
{
    Suite *s = suite_create("db_migration_size");

    TCase *tc_core = tcase_create("Core");
    tcase_set_timeout(tc_core, IK_TEST_TIMEOUT);
    tcase_add_checked_fixture(tc_core, migration_test_setup, NULL);
    tcase_add_test(tc_core, test_migration_file_too_large);
...
int main(void)
{
    ...
    srunner_set_xml(sr, "reports/check/unit/apps/ikigai/db/zzz_migration_size_test.xml");
