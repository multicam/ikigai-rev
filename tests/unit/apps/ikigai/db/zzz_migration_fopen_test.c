...
// Test: fopen failure (line 70)
START_TEST(test_migration_fopen_failure) {
    ...
    // Create migration file - our mock fopen will fail to open it
    create_migration_file(test_dir, "0001-init.sql", "SELECT 1;");

    // Should fail because fopen returns NULL
...
{
    Suite *s = suite_create("db_migration_fopen");

    TCase *tc_core = tcase_create("Core");
    tcase_set_timeout(tc_core, IK_TEST_TIMEOUT);
    tcase_add_checked_fixture(tc_core, migration_test_setup, NULL);
    tcase_add_test(tc_core, test_migration_fopen_failure);
...
int main(void)
{
    ...
    srunner_set_xml(sr, "reports/check/unit/apps/ikigai/db/zzz_migration_fopen_test.xml");
