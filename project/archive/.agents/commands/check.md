...
### 1. Initial Check
Run `make check`. If it passes, report success and stop.
If it fails, identify all files with test failures.

### 2. Fix Issues With Sub-Agents

**Scope:** One sub-agent per file. Each sub-agent fixes ALL test failures in its assigned file.
...
### 3. Verify Results
After ALL sub-agents have completed, re-run `make check`.
- If it passes: report success
- If issues remain: compare to initial count

### 4. Progress Evaluation
If issues remain after a full pass:
- **Forward progress** = fewer total test failures than before this pass
