Run helgrind thread checks and fix issues using sub-agents.

**Usage:**
- `/helgrind` - Run make check-helgrind and dispatch sub-agents to fix thread issues

**Action:** Executes `make check-helgrind` and uses sub-agents with the coverage persona to fix any thread errors (races, lock issues) found. Sub-agents run sequentially (one per file) and continue until all issues are resolved.
...
### 1. Initial Check
Run `make check-helgrind`. If it passes, report success and stop.
If it fails, identify all files with thread errors.

### 2. Fix Issues With Sub-Agents

**Scope:** One sub-agent per file. Each sub-agent fixes ALL thread errors in its assigned file.
...
- All thread errors found in that file
...
### 3. Verify Results
After ALL sub-agents have completed, re-run `make check-helgrind`.
- If it passes: report success
- If issues remain: compare to initial count

### 4. Progress Evaluation
If issues remain after a full pass:
- **Forward progress** = fewer total thread errors than before this pass
