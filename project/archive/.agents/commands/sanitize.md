Run sanitizer tests and fix failures using sub-agents.
...
### 1. Initial Check
Run `make check-sanitize`. If it passes, report success and stop.
If it fails, identify all files with sanitizer errors (ASan, UBSan).

### 2. Fix Issues With Sub-Agents

**Scope:** One sub-agent per file. Each sub-agent fixes ALL sanitizer errors in its assigned file.
...
### 3. Verify Results
After ALL sub-agents have completed, re-run `make check-sanitize`.
- If it passes: report success
- If issues remain: compare to initial count

### 4. Progress Evaluation
If issues remain after a full pass:
- **Forward progress** = fewer total sanitizer errors than before this pass
